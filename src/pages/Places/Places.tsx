...
import {DocumentWithId, IDataFirebase, IGeoData} from "../../utils/interfaces";
...
export const Places = () => {
  ...
  const removeFromDataUser = (indexToRemove: number) => {
    ...
    if (dataUserString !== null) {
      const dataUser: { fLat: string; fLng: string; fNam: string; fDes: string }[] = JSON.parse(dataUserString);
